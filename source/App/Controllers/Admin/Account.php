...
class Account extends AdminViewOnly
{
        ...
        if ($this->request->isPost() && !$errors) {
            ...
            switch ($body['submit']) {
                ...
                case $submit_buttons[1]:
                    ...
                    if ($cleaned['username'] === $this->user->LoginUsername) {
                ...
                case $submit_buttons[2]:
                    ...
                    if ($cleaned['email'] === $this->user->LoginEmail) {
    ...
    {
        if (!($new_password && $old_password) || !password_verify($old_password, $this->user->LoginHashedPassword)) {
            return 'Incorrect password';
        }
        $hashed = password_hash($new_password, 1);
        return $this->user->updatePassword($hashed) ? true : 'An error has occurred while trying to update your password. Please try again later.';
    }
    ...
    private function updateUsername($username)
    {
        ...
        $result = false;
        try {
            $result = $this->user->updateUsername(strip_tags($username));
    ...
    private function updateEmail($email)
    {
        ...
        $result = false;
        try {
            $result = $this->user->updateEmail(strip_tags($email));
        } catch (PDOException $e) {
            if (intval($e->errorInfo[1]) === 1062) {
